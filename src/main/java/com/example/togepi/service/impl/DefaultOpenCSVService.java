...
public class DefaultOpenCSVService implements OpenCSVService {
    ...
    public List<User> readToObject() throws ResourceNotFoundException {
        try {
            ...
            final List<User> users = csvUsers.stream().map(csvUser -> mapper.map(csvUser, User.class)).collect(Collectors.toList());
    ...
    public List<User> writeFromObject() throws ResourceNotFoundException {
        try {
            ...
            final List<CSVUser> csvUsers = users.stream().map(user -> mapper.map(user, CSVUser.class)).collect(Collectors.toList());
